...
void Draw_Character (int x, int y, int num)
{
	...
	// FIXME: pre-expand to native format?
		...
		{
			if (source[0])
				pusdest[0] = GET_SWIFT_d_8to16table()[source[0]];
			if (source[1])
				pusdest[1] = GET_SWIFT_d_8to16table()[source[1]];
			if (source[2])
				pusdest[2] = GET_SWIFT_d_8to16table()[source[2]];
			if (source[3])
				pusdest[3] = GET_SWIFT_d_8to16table()[source[3]];
			if (source[4])
				pusdest[4] = GET_SWIFT_d_8to16table()[source[4]];
			if (source[5])
				pusdest[5] = GET_SWIFT_d_8to16table()[source[5]];
			if (source[6])
				pusdest[6] = GET_SWIFT_d_8to16table()[source[6]];
			if (source[7])
				pusdest[7] = GET_SWIFT_d_8to16table()[source[7]];
...
void Draw_Pic (int x, int y, qpic_t *pic)
{
	...
	// FIXME: pretranslate at load time?
		...
		for (v=0 ; v<pic->height ; v++)
		{
			...
			{
				pusdest[u] = GET_SWIFT_d_8to16table()[source[u]];
			}
	...
	// FIXME: pretranslate at load time?
		...
		for (v=0 ; v<pic->height ; v++)
		{
			for (u=0 ; u<pic->width ; u++)
			{
				...
				{
					pusdest[u] = GET_SWIFT_d_8to16table()[tbyte];
				}
			}
...
// draw the pic
	...
	else
	{
		...
		// FIXME: does the endian switching go away in production?
			...
			for (x=0 ; x<vid.conwidth ; x+=4)
			{
				pusdest[x] = GET_SWIFT_d_8to16table()[src[f>>16]];
				f += fstep;
				pusdest[x+1] = GET_SWIFT_d_8to16table()[src[f>>16]];
				f += fstep;
				pusdest[x+2] = GET_SWIFT_d_8to16table()[src[f>>16]];
				f += fstep;
				pusdest[x+3] = GET_SWIFT_d_8to16table()[src[f>>16]];
				f += fstep;
			}
		}
	}
...
// FIXME: would it be better to pre-expand native-format versions?
	...
	if (transparent)
	{
		for (i=0 ; i<prect->height ; i++)
		{
			for (j=0 ; j<prect->width ; j++)
			{
				...
				if (t != TRANSPARENT_COLOR)
				{
					*pdest = GET_SWIFT_d_8to16table()[t];
				}
	...
	else
	{
		for (i=0 ; i<prect->height ; i++)
		{
			for (j=0 ; j<prect->width ; j++)
			{
				*pdest = GET_SWIFT_d_8to16table()[*psrc];
				psrc++;
				pdest++;
			}
...
void Draw_Fill (int x, int y, int w, int h, int c)
{
	...
	{
        short* t = GET_SWIFT_d_8to16table();
		uc = t[c];
